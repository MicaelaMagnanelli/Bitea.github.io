    ...
    function actualizarVistaCupcakes() {
        const cupcakesHtml = Array.from({ length: cupcakeCount }, (_, index) => {
            ...
            if (window.innerWidth <= 414) {
                top = 500 + index * 50; // Ajuste para móviles pequeños
                left = 0;
            } else if (window.innerWidth <= 820) {
                top = 880 + index * 105; // Ajuste para tablets medianas
                left = 70;
            } else if (window.innerWidth <= 1024) {
                top = 300 + index * 50; // Ajuste para pantallas de 768 a 1024px
                left = 600;
            } else {
                top = 200 + index * 50; // Ajuste para pantallas grandes
                left = 1300;
            }
        ...
        // Ajuste de la posición del botón de agregar +
        if (window.innerWidth <= 414) {
            ...
            agregarButton.style.position = "absolute";
            agregarButton.style.left = "100px";
            agregarButton.style.top = `${850 + cupcakeCount * 55 + 30}px`;
        } else if (window.innerWidth <= 1024) {
            agregarButton.style.position = "absolute";
            agregarButton.style.left = "600px";
            agregarButton.style.top = `${300 + cupcakeCount * 50 + 30}px`;
        } else {
            agregarButton.style.position = "absolute";
