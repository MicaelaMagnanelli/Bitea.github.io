...
// GLUTEN FREE Y KETO
// Obtener los elementos de los botones, la barra y la imagen
...
// Rutas de las imágenes para cada dieta
const ketoImage = 'Imagenes/Keto y Gluten Free/15. C. KETO SIN FONDO.png'; // Corregir ruta de la imagen de Keto
const glutenFreeImage = 'Imagenes/Keto y Gluten Free/14. C. GLUTEN FREE (SIN FONDO).png'; // Corregir ruta de la imagen de Gluten Free
...
// Función para cambiar la imagen, el texto y mover la barra
function updateContent(buttonClicked) {
    if (buttonClicked === 'keto') {
        imageDisplay.src = ketoImage; // Cambiar imagen a Keto
        title.textContent = 'Keto'; // Cambiar título a "Keto"
        description.textContent = ketoDescription; // Cambiar descripción
        indicator.style.width = '50%'; // Mover la barra debajo de "Keto"
    } else if (buttonClicked === 'glutenFree') {
        imageDisplay.src = glutenFreeImage; // Cambiar imagen a Gluten Free
        title.textContent = 'Gluten Free'; // Cambiar título a "Gluten Free"
        description.textContent = glutenFreeDescription; // Cambiar descripción
        indicator.style.width = '100%'; // Mover la barra debajo de "Gluten Free"
    }
    ...
    if (abrirPopup) {
        ...
        const continuarHandler = () => {
            if (etapaActual < etapas.length - 1) {
                ...
                alert('¡Cupcake personalizado completado!');
                cerrarPopupHandler();
            }
        };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const popupPersonalizado = document.getElementById('popup-personalizado');
    const abrirPopupPersonalizado = document.getElementById('abrir-popup-personalizado');
    const cerrarPopupPersonalizado = document.getElementById('cerrar-popup-personalizado');
    
    // Mostrar el pop-up cuando se haga clic en "Agregar +"
    abrirPopupPersonalizado.addEventListener('click', () => {
        popupPersonalizado.classList.add('active'); // Mostrar el pop-up
    });

    // Cerrar el pop-up cuando se haga clic en la "X"
    cerrarPopupPersonalizado.addEventListener('click', () => {
        popupPersonalizado.classList.remove('active'); // Ocultar el pop-up
    });
});
